...
module.exports = (app) => {
  const { client, clients, url, options, defaultDB, customPromise, loadModel, modelPath, plugins } = app.config.mongoose;
  ...
  app.mongoose.loadModel = () => loadModelToApp(app, modelPath);

  if (loadModel) {
    app.beforeStart(() => {
      loadModelToApp(app, modelPath);
    });
  }
};
...
function loadModelToApp(app, modelPath = "app/model") {
  const dir = path.join(app.config.baseDir, modelPath);
